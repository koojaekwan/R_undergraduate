...
표본평균을 구할 때 첫번째 표본을 모집단으로 생각해 중복이 허용되는 복원추출로 표본을 뽑아낸다.
이 작업을 B번만큼 반복한다. 즉, B개의 데이터 셋이 생긴다.  
비슷한 데이터 셋 또는 계산된 통계량의 값이 비슷할수도 다를수도 있다.  

우리는 이 것들을 일반화를 시켜서 이야기 해야한다. `점추정`과 `구간추정`으로 이야기 해줄 수 있다.
보통은 모수에 대한 추정을 할 때, 치중되는 방향은 점추정이다. 하지만 여기서는 구간추정을 중요시한다.  
...
95%보다는 90%가 더 안정적일 수 있다. 조금 위험한 경우는 99%인경우인데, 99%보다는 95%쓰는 이유가
이상치가 포함될 수 있을경우 때문이다.  
...
그래서 점추정뿐만아니라, 구간추정도 해보자.
200개 각각보다 일반적인 상황을 이야기해야 하므로 `평균`을 보자.  
